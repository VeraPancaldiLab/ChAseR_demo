...
##Define paths:
netpath='/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Common/forGARDEN-NET/BP/PCHiCdataBP'
...
```{r}
###Randomization
rchromneunet_bb <- chaser::randomize(chromneunet_bb, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
rnetmetneu_bbcomp <- chaser::randomize(netmetneu_bbcomp, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
###Tcells
rtexp <- chaser::randomize(tnetbbexp, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
plot(colMeans(rtexp_feat[[1]]),rtexp_chas[[1]], pch='.', ylim=c(0.02,0.13),xlim=c(3., 3.2), col=mycols, xlab='Mean expression Tcells', ylab='ChAs on PP network') 
for (i in 2:nrand){
...
plot(colMeans(rpneuexp_feat[[1]]),rpneuexp_chas[[1]], pch='.', col=mycols, ylim=c(-0.05,0.15),xlim=c(2.9, 3),  xlab='Mean expression neutrophils', ylab='ChAs on PP network', axes=F) 
for (i in 2:nrand){
...
Beekpath='/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Common/forGARDEN-NET/Beekman2018_BcellsEpigenome/converted_to_hg19'
fs=list.files(Beekpath)

for (f in fs ){
  featn=gsub('.vst.txt', '',f)
  beek=read.table(paste(Beekpath,f,sep='/'), header=T)
...
beekprocnew=data.frame(beek[,c(1,2,3)], rowMeans(beek[,c(4:ncol(beek))]))

rownames(beekprocnew)=NULL
colnames(beekprocnew)[4]=featn


netbpB=load_features(netbpB, beekprocnew, type='features_table',featnames=featn,auxfun=mean, missingv=0)
}
...
rnetbpB=randomize(netbpB,nrandom=nrand, preserve.nodes=NULL, dist.match=T)
rnetbpB_feat=lapply(rnetbpB, export)
rnetbpB_chas=lapply(rnetbpB, chas)


rnetbpB_bb=chaser::randomize(netbpB_bb, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
rnetbpB_bbcrosschasfin=Reduce('+', rnetbpB_bbcrosschas)/nrand

rnetbpB_bbandchas=lapply(rnetbpB_bb, chas, method='and')
rnetbpB_bbandchasfin=Reduce('+', rnetbpB_bbandchas)/nrand
...
pheatmap(netbpB_bbcrosschas,display_numbers = T, main='PP cross', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()

#pdf(paste(figurespath,'BeekBcellPCHiC_andchasPP.pdf', sep='/'))
pheatmap(netbpB_bbandchas,display_numbers = T, main='PP AND', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()

#pdf(paste(figurespath,'BeekBcellPCHiC_crosschasPO.pdf', sep='/'))
pheatmap(netbpB_bocrosschas,display_numbers = T, main='PO cross', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()

#pdf(paste(figurespath,'BeekBcellPCHiC_andchasPO.pdf', sep='/'))
pheatmap(netbpB_boandchas,display_numbers = T, main='PO AND', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()

#pdf(paste(figurespath,'BeekBcellPCHiC_crosschasPPrandom.pdf', sep='/'))
pheatmap(rnetbpB_bbcrosschasfin,display_numbers = T, main='randomPP cross', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()

#pdf(paste(figurespath,'BeekBcellPCHiC_andchasPPrandom.pdf', sep='/'))
pheatmap(rnetbpB_bbandchasfin,display_numbers = T, main='randomPP AND', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()

###try with z score
#pdf(paste(figurespath,'BeekBcellPCHiC_crosschasPPZscore.pdf', sep='/'))

pheatmap((rnetbpB_bbcrosschasfin-netbpB_bbcrosschas),display_numbers = T, main='Zscore PP cross', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()

#pdf(paste(figurespath,'BeekBcellPCHiC_andchasPPZscore.pdf', sep='/'))

pheatmap((rnetbpB_bbandchasfin-netbpB_bbandchas),display_numbers = T, main='Zscore PP AND', cluster_rows=F, cluster_cols=F, fontsize_number=14,fontsize=14, number_color = 'black',color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks=breaksList)
#dev.off()
...
```{r}
#svg(paste(figurespath, 'Figure3ABCD.svg', sep='/'), height=5, width=9)
par(mfrow=c(1,2))
plot(colMeans(netbpB_bbfeat),netbpB_bbchas,   ylab='ChAs PP',  xlab='Abundance PP',   ylim=c(0,0.43),xlim=c(0,7), pch=21, bg='red')
text(colMeans(netbpB_bbfeat),netbpB_bbchas, label=colnames(netbpB_feat), cex=0.8, pos=1)
for (i in 2:nrand){
...
plot(netbpB_bbchas,netbpB_bochas,   ylab='ChAs PO',  xlab='ChAs PP', pch=21, bg='red',  ylim=c(-0.25,0.32), xlim=c(0,0.5))
text(netbpB_bbchas,netbpB_bochas, label=colnames(netbpB_feat), cex=0.8, pos=3)
abline(a=0, b=1)
abline(h=0)
...
plot(crosschasmatsel[,3], andchasmatsel[,3], xlab='Cross ChAs of f1,f2', ylab='Chas of f1 AND f2', xlim=c(-0.2,0.4), ylim=c(0, 0.4), bg=cols, pch=21)
abline(a=0, b=1)
for (i in 2:nrand){
...
#svg(paste(figurespath,'NBeek_ChasvsAbAll.svg', sep='/'))
plot(colMeans(netbpB_feat),netbpB_chas,   ylab='ChAs',  xlab='Ab',  col='red', ylim=c(-0.15,0.4),xlim=c(0,4), main='PCHiC Bcells Beekman 2018')
for (i in 2:nrand){
...
names(netbpB_bbcrossAb)=rownames(crosschasmatsel)
...
rnetbpB_bbcrossAb=lapply(rnetbpB_bbfeat,function(y){
  ...
  #print(res)
names(res)=rownames(crosschasmatsel)
  return(res)
...
plot(netbpB_bbcrossAb, crosschasmatsel[,3], xlab='Abundance mean(f1,f2)', ylab='Cross ChAs', xlim=c(1,7), ylim=c(-0.15, 0.4), col=cols)
text(netbpB_bbcrossAb, crosschasmatsel[,3], labels=gsub('H3K', '',names(netbpB_bbchas)), cex=0.7, pos=3)
#dev.off()
for (i in 2:nrand){
 points( rnetbpB_bbcrossAb[[i]], rcrosschasmat[[i]][,3], pch='.', col=cols) 
}
#dev.off()
...
#save(fithum05net, file='/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Common/forGARDEN-NET/HiCData/fithum05netnrandk.Rdata')
...
plot(density(log10(export(fithumnet, 'edges')$dist)), type='l', xlim=c(3,8), col='grey', xlab='Distance spanned (log10 bases)')
points(density(log10(export(fithum05net, 'edges')$dist)),type='l',)
points(density(log10(export(netbpB, 'edges')$dist), na.rm=T), type='l', col='red')
points(density(log10(export(neunet, 'edges')$dist), na.rm=T),  col='blue',type='l')
#dev.off()
...
###randomization
rfithum05netHist=chaser::randomize(fithum05netHist, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
plot(colMeans(fithum05netHist_feat),fithum05netHist_chas,   ylab='ChAs ',  xlab='Abundance',  col='red', ylim=c(0,0.3),xlim=c(0.015,0.037), main='FitHiC_GM06990histmods')
...
```{r}
###randomize
rfithum05netChip=chaser::randomize(fithum05netChip, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
plot(colMeans(fithum05netChip_feat),fithum05netChip_chas,   ylab='ChAs ',  xlab='Abundance',  col='red', ylim=c(0,0.4),xlim=c(1,4), main='FitHiC GM06990 Bcells Beekman 2018')
text(colMeans(fithum05netChip_feat),fithum05netChip_chas, label=colnames(fithum05netChip_feat), cex=0.7, pos=3)
points(colMeans(rfithum05netChip_feat[[1]]),rfithum05netChip_chas[[1]], pch='.',  ylab='ChAs FitHiChum05',  xlab='Abundance', col='grey') 
...
```{r}
###compare FitHiC with PCHiC Ab and Chas
#svg(paste(figurespath, 'SuppFigs/CompareFitHiCPCHiC_All.svg', sep='/'), heigh=9, width=6) 
par(mfrow=c(3,2))
##fithic compared with  PCHiC abundance
#svg(paste(figurespath, 'SuppFigs/CompareFitHiCPCHiC_BcellsAb.svg', sep='/')) 

    plot(colMeans(netbpB_feat),colMeans(fithum05netChip_feat),   ylab='Ab FitHiC',  xlab='Ab PCHiC',  col='red', ylim=c(0,4),xlim=c(0,4) )
...
plot(netbpB_chas,fithum05netChip_chas,   ylab='ChAs FitHiC',  xlab='ChAs PCHiC',  bg='red', ylim=c(0,0.4),xlim=c(-0.2,0.4))
text(netbpB_chas,fithum05netChip_chas, label=colnames(fithum05netChip_feat), cex=0.7, pos=1)
...
#svg(paste(figurespath, 'SuppFigs/CompareFitHiCPCHiC_BcellsAb_PP.svg', sep='/')) 
 plot(colMeans(netbpB_bbfeat),colMeans(fithum05netChip_feat),   ylab='Ab FitHiC',  xlab='Ab PCHiC PP',  col='red', ylim=c(0,7),xlim=c(0,5))
...
plot(netbpB_bbchas,fithum05netChip_chas,   ylab='ChAs FitHiC',  xlab='ChAs PCHiC PP',  col='red', ylim=c(0,0.5),xlim=c(0,0.5))
...
#   svg(paste(figurespath, 'SuppFigs/CompareFitHiCPCHiC_BcellsAb_PO.svg', sep='/')) 
    plot(colMeans(netbpB_bofeat),colMeans(fithum05netChip_feat),   ylab='Ab FitHiC',  xlab='Ab PCHiC PO',  col='red', ylim=c(0,4),xlim=c(0,4))
...
abline(v=0)
 ...
 #svg(paste(figurespath,'CompareFitHiCPCHiC_BcellsChAsPO.svg', sep='/'))
plot(netbpB_bochas,fithum05netChip_chas,   ylab='ChAs FitHiC',  xlab='ChAs PCHiC PO',  bg='red', ylim=c(0,0.4),xlim=c(-0.3,0.4))
text(netbpB_bochas,fithum05netChip_chas, label=colnames(fithum05netChip_feat), cex=0.7, pos=1)
#points(rnetbpB_bochas[[1]],rfithum05netChip_chas[[1]], pch='.', col='grey') 
#for (i in 2:length(rfithum05netChip)){
# points(rnetbpB_chas[[i]],rfithum05netChip_chas[[i]], pch='.') 
#}
abline(a=0, b=1)
abline(h=0)
abline(v=0)
#text(rnetbpB_bochas[[1]],rfithum05netChip_bochas[[1]], label=colnames(fithum05netChip_feat), cex=0.7, col='blue')
...
```{r}
 #svg(paste(figurespath,'CompareFitHiCPCHiC_BcellsChAs_PP.svg', sep='/'))
plot(netbpB_bbchas,fithum05netChip_chas,   ylab='ChAs FitHiC',  xlab='ChAs PCHiC PP', bg='red', ylim=c(0,0.5),xlim=c(0,0.5))
text(netbpB_bbchas,fithum05netChip_chas, label=colnames(fithum05netChip_feat), cex=1, pos=3)
points(rnetbpB_bbchas[[1]],rfithum05netChip_chas[[1]], pch='.', col='grey') 
for (i in 2:length(rfithum05netChip)){
 points(rnetbpB_bbchas[[i]],rfithum05netChip_chas[[i]], col='grey') 
}
abline(a=0, b=1)
abline(h=0)
abline(v=0)
...
fithum05Met=make_chromnet(fithum05netmat)
dnametpath='/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Common/forGARDEN-NET/HiCData/GM06990DNAmet'
dnamet=read.table(paste(dnametpath,'GSM999353_hg19_wgEncodeHaibMethyl450Gm06990SitesRep1-beta.txt', sep='/'))
...
rfithum05Metcomp=chaser::randomize(fithum05Metcomp, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
plot(colMeans(fithum05Metcomp_feat),fithum05Metcomp_chas,   ylab='ChAs ',  xlab='Abundance',  bg='red',pch=21,xlim=c(0.4945,0.5),ylim=c(-0.05,0.05), main='FitHiC_GM06990DNAmet')
...
```{r}
expdatapath='/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Common/forGARDEN-NET/HiCData/GM06990RNAseq/'

exphum=read.table(paste(expdatapath,'GSM2040732_GM06990_genes.fpkm_tracking',sep='/'), sep='\t', header=T)
...
rfithum05netExp=chaser::randomize(fithum05netExp, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
rneunetRT=chaser::randomize(neunetRT, nrandom=nrand, preserve.nodes = NULL,dist.match = T)
...
plot(colMeans(neunetRTfeat),neunetRTchas,   ylab='ChAs RT ',  xlab='RT mean', ylim=c(0,0.8),xlim=c(1.2, 1.5),  bg='red', pch=21, main='Neunet')
...
bpnet=make_chromnet(bpproc[,c(1,2,3,6,7,8)])
baits <- export(bpnet, 'baits')
nodes=export(bpnet, "nodes")$name
oes=nodes[-which(nodes %in% baits)]
...
#measlist=list( unlist(bppromdeg), unlist(bppromoedeg),unlist(bppromTrans), unlist(bppromlLCC), unlist(bppromNclus))
...
#names(measlist)=c( 'PP Degree', 'PO P degree','Clustering Coefficient.', '% in Largest CC Component','Number of CCs' )
...
#svg(paste(figurespath,'Netstats_new2.svg', sep='/'), height=7, width=11)                        
...
#bpproml1_mod=cluster_fast_greedy(Gbpproml[[1]], merges = TRUE, modularity = TRUE,  membership = F, weights = NULL)
